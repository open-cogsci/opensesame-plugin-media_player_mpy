...
class OpenGLRenderer(object):
    ...
    def __init__(self):
        raise osexception(
            "This class should only be subclassed on not be instantiated directly!")
    ...
    def prepare_for_playback(self):
        ...
        # Set screen coordinates to useful values for movie playback (per pixel
        # coordinates)
        GL.glMatrixMode(GL.GL_PROJECTION)
        GL.glPushMatrix()
        GL.glLoadIdentity()
        GL.glOrtho(0.0, self.main_player.experiment.var.width,
                   self.main_player.experiment.var.height, 0.0, 0.0, 1.0)
    ...
    def draw_frame(self):
        ...
        # Make sure there are no pending drawing operations and flip front and
        # backbuffer
        GL.glFlush()
